...
# MAX_AGENTIC_ATTEMPTS = int(os.getenv("MAX_AGENTIC_ATTEMPTS", "2"))
...
#     agentic_pipeline_current_counter : int
...
#     if state.get("agentic_pipeline_current_counter", 0) > 0:
#         required_attempts = 1
#     else:
#         required_attempts = max_attempts

#     if state.get("clarification_attempts", 0) >= required_attempts:
#         logger.info(f"🚀 AGENTIC PIPELINE TRIGGERED (Attempts: {state.get('clarification_attempts', 0)}/{required_attempts})")
...
# #     from agenticPipeline.crewai_agents import Message as AgenticMessage, Role as AgenticRole, run_orchestration

# #     all_messages = state.get("messages", [])
# #     if not all_messages:
# #         logger.warning("trigger_agentic_group called with no messages in state")
# #         return {
# #             "messages": [AIMessage(content="AGENTIC PIPELINE HAS NOT CAME INTO ACTION")],
# #             "clarification_attempts": 0
# #         }

# #     current_user_message = state.get("rewrittenQueryforVectorsearch")
...
# #             current_user_message = str(all_messages[-1].content)

# #     history_messages = all_messages[:-1] if all_messages[-1] is not current_user_message else all_messages[:-1]
# #     conversation_history = [
# #         AgenticMessage(
# #             role=AgenticRole.USER if isinstance(m, HumanMessage) else AgenticRole.ASSISTANT,
# #             content=str(m.content),
# #         )
# #         for m in history_messages
# #     ]

# #     logger.info(f"Handing off to agentic pipeline | current_message={current_user_message!r} | history_len={len(conversation_history)}")
...
# #             return {
# #                 "messages": [AIMessage(content="AGENTIC PIPELINE HAS CAME INTO ACTION — but classification failed, please try rephrasing.")],
# #                 "clarification_attempts": 0
# #             }

# #         decision = flow.state.decision
# #         plan = flow.result()
# #         logger.info(f"Agentic pipeline decision: {decision}")
# #         logger.info(f"Agentic pipeline plan: {plan}")
# #         collection_name = state.get("collection_name")

# #         from container import get_container
# #         container = get_container()
# #         if plan and container.vector_db.ensure_collection_exists(collectionName=collection_name):
# #             retrieved_docs = container.vector_db.batch_search_with_top_k_grouping(
# #                 agent_plan_data=plan,
# #                 collectionName=collection_name
# #             )
# #             realResult = await container.llm.filteringresultwithLLM(retrieved_docs, current_user_message)
# #             final_content = realResult.output
            
# #         return {
# #             "messages": [AIMessage(content=f"AGENTIC PIPELINE HAS CAME INTO ACTION\n\nDecision: {decision.model_dump_json()}\nPlan: {plan.model_dump_json()}")],
# #             "clarification_attempts": 0
# #         }

# #     except Exception as e:
# #         logger.error(f"Agentic pipeline invocation failed: {e}", exc_info=True)
# #         return {
# #             "messages": [AIMessage(content="AGENTIC PIPELINE HAS CAME INTO ACTION — but an error occurred while running it.")],
# #             "clarification_attempts": 0
# #         }
# async def trigger_agentic_group(state: GraphState, runtime: Runtime) -> dict[str, Any]:
#     logger.info("Inside trigger_agentic_group")
...
#     current_counter = state.get("agentic_pipeline_current_counter") or 0
#     if current_counter > MAX_AGENTIC_ATTEMPTS :
#         return {
#             "messages" : [AIMessage(content = "So many attempts of the agentic pipeline reached")],
#             "document_found": False,
#             "clarification_attempts": 0,
#             "agentic_pipeline_current_counter": 0
#         }
...
#                 collectionName=collection_name
#             )
#             logger.info(f"🚀 Retrieved {len(retrieved_docs)} unique chunks across agentic sub-queries.")

#             realResult = await container.llm.filteringresultwithLLM(retrieved_docs, current_user_message)
#             final_content = realResult.output
#             is_found = realResult.isAnswerFound

#             if is_found :
#                 return {
#                     "messages": [AIMessage(content = final_content)],
#                     "document_found": is_found,
#                     "clarification_attempts": 0,
#                     "agentic_pipeline_current_counter" : 0
#                 }
#             else :
...
#                     "document_found": is_found,
#                     "clarification_attempts": 0,
#                     "agentic_pipeline_current_counter" : state.get("agentic_pipeline_current_counter", 0) + 1
#                 }

#         return {
#             "messages": [AIMessage(content="Try rephrasing your query")],
#             "document_found": False,
#             "clarification_attempts": 0,
#             "agentic_pipeline_current_counter": state.get("agentic_pipeline_current_counter", 0) + 1
#         }
...
#         logger.error(f"Agentic pipeline invocation failed: {e}", exc_info=True)
#         return {
#             "messages": [AIMessage(content="An error occurred while executing the multi-query retrieval.")],
#             "document_found": False,
...
class GraphState(MessagesState):
    ...
    request_id : str
    agent_name : str
    maximum_agentic_effort_reached : bool = False
...
def _log_knowledge_gap(session_id: str, request_id: str, raw_message: str, collection_name: str) -> None:
    """Asynchronously records unresolved queries to Firestore for the hourly SRE digest."""
    try:
        from container import get_container
        ...
        db.collection("knowledge_gaps").add({
            ...
            "status": "pending_hourly_digest",
            "timestamp": datetime.now(timezone.utc).isoformat(),
        })
        logger.info(f"📝 Logged unanswered query to 'knowledge_gaps' collection for request {request_id}")
    except Exception as e :
...
async def tool_node(state: GraphState, runtime: Runtime) -> dict[str, Any]:
    ...
    is_found = realResult.isAnswerFound
    new_attempts = 0 if is_found else state.get("clarification_attempts", 0) + 1

    logger.info(f"realResult output is {realResult.output}")
    logger.info(f"realResult isAnswerFound is {is_found}")
    logger.info(f"Clarification attempts is {state.get('clarification_attempts', 0)}")
    logger.info(f"New attempt currently going is {new_attempts}")
    ...
    return {
        ...
        "document_found": is_found,
        "clarification_attempts": new_attempts,
        "agentic_pipeline_current_counter": 0 if is_found else state.get("agentic_pipeline_current_counter", 0)
    }
...
async def trigger_agentic_group(state: GraphState, runtime: Runtime) -> dict[str, Any]:
    ...
    current_user_message = state.get("rewrittenQueryforVectorsearch")
    logger.info(f"Constructed query for the rag search is {current_user_message}")
    if not current_user_message or not current_user_message.strip():
        for m in reversed(all_messages):
            if isinstance(m, HumanMessage):
                current_user_message = m.content
    ...
    if current_counter > MAX_AGENTIC_ATTEMPTS:
        try:
            _log_knowledge_gap(
                    ...
                    raw_message=current_user_message,
                    collection_name=state.get("collection_name", "")
                )
            
            from agenticPipeline.crewai_agents.web_research_crew import WebResearchCrew
            
            research_crew = WebResearchCrew()
            web_answer = await asyncio.to_thread(
                research_crew.execute_research,
                user_query=current_user_message
            )
            
            return {
                "messages": [AIMessage(content=f"Here is verified information found from public web research:\n\n{web_answer}")],
                "document_found": True,
                "clarification_attempts": 0,
                "agentic_pipeline_current_counter": 0,
                "agent_name" : "websearch",
                "maximum_agentic_effort_reached" : True
            }
        except Exception as exc:
            logger.error(f"Last-resort web research failed: {exc}", exc_info=True)
            return {
                "messages": [AIMessage(content="I have searched our internal records and public web sources, but could not find verified details for this query. Please contact customer support.")],
                "document_found": False,
                "clarification_attempts": 0,
                "agentic_pipeline_current_counter": 0
            }
    ...
    try:
        ...
        try :
            from agenticPipeline.crewai_agents import SQSMetricsEmitter
            emitter = SQSMetricsEmitter()
            emitter.emit(
                state = flow.state,
                session_id = state.get("session_id", "unknown"),
                request_id = state.get("request_id") or f"req_{uuid.uuid4().hex[:8]}",
            )
            logger.info(f"Successfully emitted live metrics to SQS for session and request id : {state.get('session_id')} and {state.get('request_id')}")
        except Exception as e :
            logger.warning(f"Could not emit SQS telemetry from LangGraph node: {e}")
        ...
        if plan:
            ...
            if is_found:
                search_type = flow.state.decision.search_type.value if flow.state.decision else "unknown"
                resolved_agent = f"{search_type}Agent"
                logger.info(f"🎯 Query successfully resolved by agentic specialist: {resolved_agent}")
                return {
                    "messages": [AIMessage(content=final_content)],
                    "document_found": is_found,
                    "clarification_attempts": 0,
                    ...
                    "maximum_agentic_effort_reached" : False
                }
            else:
                return {
                    "messages": [AIMessage(content=final_content)],
